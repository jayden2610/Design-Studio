---
name: minimal-zine-poster-v0-1
description: Generate Minimal Zine Poster v0.1 poetic paper-poster prompts and the matching generated image. Use when Jayden gives a theme, sentence, object, mood, article idea, photo, or content brief and wants a quiet Japanese/Korean zine-like editorial poster with large negative space, aged paper texture, experimental typography, restrained color accents, and a generated bitmap image. Trigger: /minimal-zine-poster
---

# Minimal Zine Poster v0.1

Source: reverse-engineered from `github.com/LiamGvchi/gc-minimal-zine-poster` (Codex skill), ported to this vault's skill architecture. See `../ARCHITECTURE.md` for the reusable template this skill instantiates — use that file when reverse-engineering a *new* design style into a skill.
...
- **Style summary:** use only the visual identity and anti-identity: poetic minimal paper poster, huge negative space, old paper, tiny anchor, sparse type, one clear high-chroma anchor, zine/editorial mood.
- **Core visual rules:** use the concrete renderable rules for canvas, composition, background, image anchor, typography, color, texture, lighting, and mood.
- **Stable non-negotiables:** vertical 3:5 paper canvas, small cluster, scanned-paper view, old print defects, serif/typewriter text, and a saturated color anchor visible at thumbnail size.
- **Replaceable variables:** object, photo/cutout/silhouette/block type, accent color, text line, date/weather, position, paper tone.
- **Negative constraints:** use as negative prompt material.
- **Prompt structure template:** use its field order, not its sample wording.
...
5. Generate the image.
   - Use the qwencloud-image-generation skill (or whatever image-gen tool is available) by default.
...
- "make a minimal zine poster about rainy days at an old bookstore"
- "minimal zine poster, standard mode, about an old book"
- "use this photo to make a poster in this same style"
